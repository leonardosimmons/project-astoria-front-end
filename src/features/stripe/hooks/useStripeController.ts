...
import { CartContext } from '../../../utils/types/types';
...
function useStripeController() {
  ...
  const http: AxiosInstance = axios.create({ baseURL: process.env.NEXT_PUBLIC_STRIPE_API });
  ...
  React.useEffect(() => {
    http.post(process.env.NEXT_PUBLIC_CREATE_PAYMENT_INTENT_API!, { currency: 'usd', items: cart.items })
      .then((res) => res.data)
      .then((data) => {
